...
log_fname = result_dir + '/gj504_contrast_curve_log.txt'
...
#flux_arr = test_rms_arr*10
#flux_arr = test_rms_arr*10000
flux_arr = np.array([150, 50, 20, 10, 6, 4, 2, 2])
#flux_arr = np.array([3])
...
for i, (R_fp, flux_fp) in enumerate(zip(R_arr, flux_arr)):
    for j, PA_fp in enumerate(PA_arr):
        for k, K in enumerate(K_arr):
            ...
            klip_config, klip_data, coadd_detlim, med_detlim = klip_subtract(dataset_label, data_dir, result_dir, R_inner, R_out, mode_cut=[K], DPhi=[360.], Phi_0=[0.],
                                                                             fwhm=fwhm, min_refgap_fac=[refgap_fac], op_fr=None, N_proc=N_proc, diagnos_stride=diagnos_stride,
                                                                             fake_planet=(R_fp, PA_fp, flux_fp), synth_psf_img=synthpsf_img, test_mode=test_mode, use_svd=True,
                                                                             coadd_full_overlap_only=True, store_results=store_results, store_psf=False, store_archv=False,
                                                                             log_fobj=log_fobj)
